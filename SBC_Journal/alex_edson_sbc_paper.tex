...
The vertex and fragment shaders of Phong shading implement the theory described on Section \ref{phong}, where the normal values of the primitives are interpolated on the vertex shader,
 and then the lightining calculations are done in the fragment shader. The Listing \ref{codphongvs} and the Listing \ref{codphongfs} show the definitions of the vertex and fragment shaders, respectively.  
	...
	\lstinputlisting[language=C, label = {codphongvs}, frame = single, caption = {Phong Shader: vertex shader}]{codes/phong_vs.txt}

	\lstinputlisting[language=C, frame = single, caption =  {Phong Shader: fragment shader}, label = {codphongfs}  ]{codes/phong_ps.txt}
...
They are done on the vertex shader and the final values are interpolated and used by the fragment shader, as shown on Listing \ref{codgouraudvs} and Listing \ref{codgouraudfs}. 
	...
	\lstinputlisting[language=C, label = {codgouraudvs}, frame = single, caption = {Gouraud Shader:  vertex shader}]{codes/gouraud_vs.txt}

	\lstinputlisting[language=C, label = {codgouraudfs},   frame = single, caption =  {Gouraud Shader: fragment shader} ]{codes/gouraud_ps.txt}
...
\subsubsection{Red Shader}
	...
	\lstinputlisting[language=C, caption = {Red Shader: vertex shader}, frame = single, label = {codredvs}]{codes/red_vs.txt}
	
	\lstinputlisting[language=C, caption = {Red Shader:  fragment shader}, frame = single, label = {codredfs}]{codes/red_ps.txt}
...
\subsubsection{Toon Shader}
	...
	\lstinputlisting[language=C,label = {codtoonvs}, frame = single, caption = {Toon Shader:  vertex shader} ]{codes/toon_vs.txt}
  
 	\lstinputlisting[language=C, caption = {Toon Shader:  fragment shader},  frame = single, label = {codtoonfs} ]{codes/toon_ps.txt}
...
Its fragment shader (Listing \ref{codflatfs}) just defines a color to the the fragment.
	...
	\lstinputlisting[language=C,label = {codflatvs},  frame = single, caption = {Flat Shader:  vertex shader} ]{codes/flat_vs.txt}

	\lstinputlisting[language=C,  frame = single, caption = {Flat Shader:  fragment shader}, label = {codflatfs} ]{codes/flat_ps.txt}
...
Each color component is calculated using as parameter a coordinate ($x$, $y$ ou $z$) given to the function \texttt{random(vec2 v)}, which returns a random value based on the given coordinate. In the Listing \ref{codrandfs} the color of the fragment is set.
	...
	\lstinputlisting[language=C,label = {codrandvs},  frame = single, caption = {Random Color Shader:  vertex shader} ]{codes/randcolor_vs.txt}
 
	\lstinputlisting[language=C,  frame = single, caption = {Random Color Shader:  fragment shader}, label = {codrandfs} ]{codes/randcolor_ps.txt}
...
the fragment shader uses these coordinates and applies it into a texture by using the function \texttt{texture2D()}.
	...
	\lstinputlisting[language=C,  frame = single, caption =  {Simple Texture Shader:  vertex shader}, label = {codtexvs} ]{codes/simple_tex_vs.txt}

	\lstinputlisting[language=C,  frame = single, caption =  {Simple Texture Shader:  fragment shader}, label = {codtexfs} ]{codes/simple_tex_ps.txt}
...
which receives as parameters the normal vector and the texture to be mapped.
	...
	\lstinputlisting[language=C,  frame = single, caption =  {CubeMap Shader:  vertex shader}, label = {codcubemapvs} ]{codes/cubemap_vs.txt}

	\lstinputlisting[language=C,  frame = single, caption =  {CubeMap Shader:  fragment shader}, label = {codcubemapps} ]{codes/cubemap_ps.txt}
...
\subsubsection{Reflection Shader}
	...
	\lstinputlisting[language=C, frame = single, caption = {Reflection Shader:  vertex shader}, label = {codreflvs}]{codes/reflection_vs.txt}. 

	\lstinputlisting[language=C, frame = single, caption = {Reflection Shader:  fragment shader}, label = {codreflfs}]{codes/reflection_ps.txt}
...
\lstinputlisting[language=C, numbers=none, frame = single, label = {linhacomando}, caption = {Command-lines}]{codes/linhacomando.txt}
...
\lstinputlisting[language=C,basicstyle=\ttfamily, frame = single, caption = {Representation of the vertex shader execution}, label = {vertex_program}]{codes/vertex_proc.txt}
...
\lstinputlisting[language=C,basicstyle=\ttfamily, frame = single, caption = {Representation of the fragment shader execution}, label = {frag_program}]{codes/fragment_proc.txt}
...
\lstinputlisting[language=C,basicstyle=\ttfamily, frame = single, caption = {Example of vertex shader}, label = {vertex_const}]{codes/red_vs.txt}
